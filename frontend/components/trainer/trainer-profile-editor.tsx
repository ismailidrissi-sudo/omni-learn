...
import { computeProfileCompletion } from "@/lib/trainer-profile-completion";
...
function useProfileCompletion(profile: TrainerProfile): { pct: number; missing: string[] } {
  return useMemo(() => computeProfileCompletion(profile), [profile]);
}
...
export function TrainerProfileEditor() {
  ...
  return (
    <div className="min-h-[50vh]">
      <main className="max-w-5xl mx-auto px-0 sm:px-0 py-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {profile.slug && (
              <Link href={`/trainer/${profile.slug}`} target="_blank" rel="noopener noreferrer">
        ...
        {completion.pct < 80 && (
          <Card className="p-4 mb-4 border border-[#BA7517]/30 bg-[#FDF8F3] rounded-xl shadow-none">
            <p className="text-sm text-[#1A1A1A]">
              {t("trainer.profile.completionBanner", { pct: String(completion.pct) })}
            </p>
            <div className="h-2 bg-black/[0.06] rounded-full overflow-hidden mt-3">
              <div
